  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AttendanceDetail | null> => {
      ...
      const [
        ...
        safeQuery<any[]>(supabase.from("facial_maps").select("id, modality, notes, created_at, facial_map_applications(id, region, side, product_name, quantity, unit, notes)").eq("appointment_id", appointmentId).order("created_at")),
