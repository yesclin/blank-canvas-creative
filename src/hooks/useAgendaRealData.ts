  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map(apt => ({
        ...
        payment_status: (apt as any).payment_status || "pendente",
        amount_expected: (apt as any).amount_expected ?? apt.expected_value ?? 0,
        amount_received: (apt as any).amount_received ?? 0,
        amount_due: (apt as any).amount_due ?? 0,
        booking_source: (apt as any).booking_source || undefined,
        patient_snapshot_name: (apt as any).patient_snapshot_name || undefined,
        patient_snapshot_phone: (apt as any).patient_snapshot_phone || undefined,
        notes: apt.notes,
        ...
        care_mode: (apt as any).care_mode || 'presencial',
        meeting_provider: (apt as any).meeting_provider,
        meeting_link: (apt as any).meeting_link,
        meeting_id: (apt as any).meeting_id,
        meeting_password: (apt as any).meeting_password,
        meeting_status: (apt as any).meeting_status || 'nao_gerada',
        meeting_created_at: (apt as any).meeting_created_at,
        meeting_started_at: (apt as any).meeting_started_at,
        meeting_ended_at: (apt as any).meeting_ended_at,
        precheck_status: (apt as any).precheck_status || 'pendente',
        consent_telehealth_accepted: (apt as any).consent_telehealth_accepted || false,
        consent_telehealth_accepted_at: (apt as any).consent_telehealth_accepted_at,
        technical_issue_count: (apt as any).technical_issue_count || 0,
