 ...
 *
 * Analytics is a local no-op registry unless explicitly re-enabled later.
 */
const CookieConsent = () => {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => setVisible(true), 600);
  ...
  const handleAccept = () => apply("granted");
  const handleReject = () => apply("denied");

  if (!visible) return null;

  return (
    <div
      className="fixed inset-x-0 bottom-0 z-[60] p-3 sm:p-5 animate-in slide-in-from-bottom-4 fade-in duration-300"
      role="dialog"
      aria-live="polite"
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4 p-5 sm:p-6">
          ...
          <div className="flex-1 text-sm text-foreground/90 leading-relaxed">
            ...
            <p className="text-muted-foreground">
              Usamos cookies e ferramentas de analytics para entender como você
              navega e melhorar o YesClin. Nada é carregado sem o seu
