-- ============================================================
-- Consumo canônico de materiais/produtos por atendimento
-- Fonte: procedure_consumption_templates + procedure_consumption_kits
-- Estoque: inventory_batches (FEFO) / inventory_movements
-- ============================================================
...
BEGIN
  ...
  -- Idempotência: já consumido para este atendimento?
  ...
  -- Itens previstos: templates diretos + itens dos kits clínicos
  FOR r IN
  ...
  LOOP
    ...
    -- FEFO: lotes com vencimento mais próximo primeiro
    FOR b IN
    ...
    -- Sem lote disponível: registra o consumo sem lote (item sem controle de lote)
...
-- ============================================================
-- Reversão (cancelamento / reabertura do atendimento)
-- ============================================================
...
DECLARE
  ...
  v_count   integer := 0;
BEGIN
  -- Já revertido? (existe devolução para o mesmo atendimento)
  IF EXISTS (
...
GRANT EXECUTE ON FUNCTION public.process_appointment_consumption(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revert_appointment_consumption(uuid) TO authenticated;