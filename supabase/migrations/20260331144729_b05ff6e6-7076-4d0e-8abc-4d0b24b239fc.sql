...
DECLARE
  ...
  _tabs JSONB;
BEGIN
  -- Normalize slug: map 'geral' → 'geral' (keep as-is, fix CASE below)
  _resolved_slug := _specialty_slug;

  -- Find existing specialty row by slug + clinic_id
  SELECT id INTO _specialty_id
  FROM public.specialties
  WHERE slug = _resolved_slug
    AND clinic_id = _clinic_id
  LIMIT 1;

  IF _specialty_id IS NULL THEN
    RAISE EXCEPTION 'Specialty "%" not found for clinic %', _resolved_slug, _clinic_id;
  END IF;
  ...
  CASE _resolved_slug
    ...
    ELSE
      RAISE NOTICE 'No specific provisioning for specialty "%"', _resolved_slug;
      RETURN;
  END CASE;
