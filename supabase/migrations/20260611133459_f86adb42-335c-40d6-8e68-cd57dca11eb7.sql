...
-- Backfill: cria Consulta/Retorno em toda clínica que ainda não tenha
...
-- Atualiza handle_new_user para semear esses dois procedimentos em novas clínicas
...
BEGIN
  ...
  -- Procedimentos padrão (sem preço; clínica define)
  INSERT INTO public.procedures (clinic_id, name, duration_minutes, allows_return, return_days, is_active)
  SELECT new_clinic_id, 'Consulta', 30, true, 30, true
  WHERE NOT EXISTS (
    SELECT 1 FROM public.procedures
    WHERE clinic_id = new_clinic_id AND lower(name) = 'consulta'
  );

  INSERT INTO public.procedures (clinic_id, name, duration_minutes, allows_return, is_active)
  SELECT new_clinic_id, 'Retorno', 20, false, true
  WHERE NOT EXISTS (
    SELECT 1 FROM public.procedures
    WHERE clinic_id = new_clinic_id AND lower(name) = 'retorno'
  );
