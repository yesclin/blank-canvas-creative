...
-- 1) Remove default_price / default_specialty_id from appointment_types
ALTER TABLE public.appointment_types DROP COLUMN IF EXISTS default_price;
ALTER TABLE public.appointment_types DROP COLUMN IF EXISTS default_specialty_id;

-- 2) Seed system procedures (Consulta, Retorno, Encaixe) for every existing clinic
INSERT INTO public.procedures (clinic_id, name, duration_minutes, price, is_active)
SELECT c.id, 'Consulta', 30, NULL, true
FROM public.clinics c
WHERE NOT EXISTS (
  SELECT 1 FROM public.procedures p
  WHERE p.clinic_id = c.id AND lower(btrim(p.name)) = 'consulta'
);

INSERT INTO public.procedures (clinic_id, name, duration_minutes, price, is_active)
SELECT c.id, 'Retorno', 30, NULL, true
FROM public.clinics c
WHERE NOT EXISTS (
  SELECT 1 FROM public.procedures p
  WHERE p.clinic_id = c.id AND lower(btrim(p.name)) = 'retorno'
);

INSERT INTO public.procedures (clinic_id, name, duration_minutes, price, is_active)
SELECT c.id, 'Encaixe', 15, NULL, true
FROM public.clinics c
WHERE NOT EXISTS (
  SELECT 1 FROM public.procedures p
  WHERE p.clinic_id = c.id AND lower(btrim(p.name)) = 'encaixe'
);

-- 3) Update handle_new_user to seed these procedures for new clinics
...
BEGIN
  ...
  -- Procedimentos padrão (preço definido pela clínica em Configurações > Procedimentos)
  INSERT INTO public.procedures (clinic_id, name, duration_minutes, price, is_active)
  VALUES
